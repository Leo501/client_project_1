cc.Class({
    ...
    initView:function(){
        ...
        var btnConfirm=cc.find("Canvas/game/dingque/mai_or_ding/confi");
        var btnCancle=cc.find("Canvas/game/dingque/mai_or_ding/cancle");
        if(btnConfirm){
            cc.vv.utils.addClickEvent(btnConfirm,this.node,"DingQue","onConfirmOrClicked");
        }
        if(btnCancle){
            cc.vv.utils.addClickEvent(btnCancle,this.node,"DingQue","onConfirmOrClicked");
        }
    },
    ...
    onConfirmOrClicked:function(event){
      var type=0;
      if(event.target.name =="cancle"){
          console.log("cancle");
          type=2;
      } else if(event.target.name =="confi"){
          console.log("confirm");
          type=0;
      } 
